...
const validateIssue = (issue, core) => {
...
const getTargetStatusOption = (statusField, core) => {
...
const processIssueItem = async (github, projectData, issue, core) => {
  const statusField = await getStatusField(github, projectData.id)
  const targetStatusOption = getTargetStatusOption(statusField, core)
...
export const moveIssues = async ({ github, context, core }) => {
  ...
  if (!validateIssue(issue, core)) {
    return
  }

  try {
    const projectData = await getProjectData(github, process.env.PROJECT_URL)
    await processIssueItem(github, projectData, issue, core)
  } catch (error) {
