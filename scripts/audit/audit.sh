...
if [ -f pnpm-audit-known-issues ] && echo "$output" | jq '[.advisories[] | {id: .id, module_name: .module_name}]' | diff -q pnpm-audit-known-issues - > /dev/null 2>&1; then
	echo
...
echo
echo "pnpm audit --json | jq '[.advisories[] | {id: .id, module_name: .module_name}]' > pnpm-audit-known-issues"
echo
